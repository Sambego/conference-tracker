...
app.get(
  "/api/conferences",
  [authCheck, guard.check("conference:list")],
  (req, res) => {
    let userId;
...
app.get(
  "/api/conference/:id",
  [authCheck, guard.check("conference:details")],
  (req, res) => {
...
app.get(
  "/api/conference/:id/submissions",
  [authCheck, guard.check("conference:submissions")],
  (req, res) => {
    let userId;
    let conferenceId = req.params.id;
...
app.post(
  "/api/conference/:id/approvals",
  [authCheck, guard.check("conference:submit")],
  (req, res) => {
    ...
    AND talkId NOT IN (?)`;
      ...
      FROM conferences c, regions r
      WHERE c.regionId = r.id AND c.id = ?`,
          [conference.id]
        );
      })
      .then(confDetails => {
      ...
      WHERE s.talkId = t.id AND t.id IN (?) AND s.conferenceId = ?`,
          [approvedSubmissions, conferenceId]
        );
      })
      .then(talkDetails => {
        hookData.talks = talkDetails.map(t => Object.assign({}, t));
        return queryOne(
          `SELECT u.name, u.picture, u.bio, u.communityUsername, u.email FROM users u WHERE u.id = ?`,
          [userId]
        );
      })
      .then(speakerDetails => {
  ...
  (req, res) => {
    ...
    getDBUserId(req.headers)
      ...
      .then(conference => {
        ...
        return query(
          `SELECT * FROM talks WHERE id IN (SELECT talkId FROM submissions WHERE conferenceId = ? and userId = ?)`,
          [conferenceId, userId]
        );
      })
      .then(talks => {
...
app.post(
  "/api/conference",
  [authCheck, guard.check("conference:add")],
  (req, res) => {
  ...
  (req, res) => {
    ...
    getDBUserId(req.headers)
      .then(id => {
        userId = id;
        return query(
          `SELECT talkId FROM submissions WHERE userId = ? AND conferenceId = ?`,
          [userId, conferenceId]
        );
      })
      .then(submittedTalks => {
        ...
        talks.map(t => {
          if (!req.body.find(s => s.talkId === t)) {
            ...
            inserts.push(
              query(
                `DELETE FROM submissions WHERE conferenceId = ? AND userId = ? and talkId = ?`,
                [conferenceId, userId, t]
              )
            );
          }
        });
...
app.get(
  "/api/meetup/:meetupId",
  [authCheck, guard.check("meetup:details")],
  (req, res) => {
    let meetup;
...
app.put(
  "/api/meetup/approved/:meetupId",
  [authCheck, guard.check("meetup:list")],
  (req, res) => {
    let userId;
    let hookData = {};
...
app.get(
  "/api/reports/todo",
  [authCheck, guard.check("report:due")],
  (req, res) => {
    let reportsTodo = [];
    let userId;
    getDBUserId(req.headers)
