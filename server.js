...
app.get(
    "/api/conferences", [authCheck, guard.check("conference:list")],
    (req, res) => {
        let userId;
        ...
        WHERE startDate > ${now()}`;
            ...
            .then(conferences => {
                let confs = conferences.map(conference => {
                    ...
                    conf.expired =
                        conf.mySubmissions === 0 && conference.cfpDate < yesterday();
...
app.get("/api/upcoming/:week?", (req, res) => {
    ...
    let sql = `SELECT c.*, c.id _id, GROUP_CONCAT(DISTINCT u.name SEPARATOR ", ") speakers, s.status, "CONFERENCE" type
      ...
      WHERE s.conferenceId = c.id
        ...
        ${
          req.params.week
            ? "AND c.startDate < " + (now() + 7 * 24 * 60 * 60 * 1000)
            : ""
        }
      GROUP BY c.name`;
    ...
    query(sql)
      ...
      ${
        req.params.week
          ? "AND m.startDate < " + (now() + 7 * 24 * 60 * 60 * 1000)
          : ""
      }`;
...
app.get(
    "/api/conference/:id", [authCheck, guard.check("conference:details")],
    (req, res) => {
        let conferenceId = req.params.id;
        let fullConference;

        let sql = `SELECT *, id _id FROM conferences WHERE id = ?`;
        queryOne(sql, [conferenceId])
            .then(conference => {
...
app.get(
    "/api/conference/:id/submissions", [authCheck, guard.check("conference:submissions")],
    (req, res) => {
        let userId;
        let conferenceId = req.params.id;
...
app.post(
    "/api/conference/:id/approvals", [authCheck, guard.check("conference:submit")],
    (req, res) => {
    ...
    AND talkId NOT IN (?)`;
            ...
            .catch(err => console.log("Error saving approvals", err))
            .then(_ => {
                return queryOne(
      ...
      WHERE s.talkId = t.id AND t.id IN (?) AND s.conferenceId = ?`, [approvedSubmissions, conferenceId]
            ...
            .then(speakerDetails => {
                ...
                events.acceptedAtConference(
                    hookData.conference,
                    hookData.talks,
                    hookData.speaker
                );
            })
            .catch(err => console.log("Error sending to external hook", err))
            .then(() => {
                res.json(conference);
            });
    }
...
app.post(
    "/api/conference/:id/rejected", [authCheck, guard.check("conference:submit")],
    (req, res) => {
        let userId;
        let conferenceId = req.params.id;
        let hookData = {};
        ...
        getDBUserId(req.headers)
            ...
            .then(_ => {
                return queryOne(`SELECT * FROM conferences WHERE id = ?`, [
                    conferenceId
                ]);
            })
            .then(conference => {
...
app.post(
    "/api/conference", [authCheck, guard.check("conference:add")],
    (req, res) => {
...
app.put(
    "/api/conference/:id", [authCheck, guard.check("conference:add")],
    (req, res) => {
...
app.post(
    "/api/conference/:id/submissions", [authCheck, guard.check("conference:submit")],
    (req, res) => {
        let userId;
        let conferenceId = req.params.id;
        ...
        getDBUserId(req.headers)
            .then(id => {
                userId = id;
                return query(
                    `SELECT talkId FROM submissions WHERE userId = ? AND conferenceId = ?`, [userId, conferenceId]
                );
            })
            .then(submittedTalks => {
                let inserts = [];
                ...
                talks.map(t => {
                    if (!req.body.find(s => s.talkId === t)) {
                        ...
                        inserts.push(
                            query(
                                `DELETE FROM submissions WHERE conferenceId = ? AND userId = ? and talkId = ?`, [conferenceId, userId, t]
                            )
                        );
                    }
                });
...
app.get(
    "/api/meetup/:meetupId", [authCheck, guard.check("meetup:details")],
    (req, res) => {
        let meetup;
        queryOne(`SELECT * FROM meetups WHERE id = ?`, [req.params.meetupId])
            .then(data => {
                meetup = data;
...
app.put(
    "/api/meetup/approved/:meetupId", [authCheck, guard.check("meetup:list")],
    (req, res) => {
        let userId;
        let hookData = {};
        ...
        getDBUserId(req.headers)
            .then(id => {
                ...
                meetupIncomingData.startDate = new Date(
                    meetupIncomingData.startDate
                ).getTime();
                delete meetupIncomingData.userId;

                console.log("Updating meetup", meetupIncomingData);
                return query(`UPDATE meetups SET ? WHERE id = ?`, [
                    meetupIncomingData,
                    req.params.meetupId
                ]);
            })
      ...
      WHERE m.talkId = t.id
        ...
        AND m.id = ?`;
            ...
            .then(speaker => {
                ...
                return queryOne(`SELECT * FROM talks WHERE id = ?`, [
...
app.post(
    "/api/meetup/applied", [authCheck, guard.check("meetup:list")],
    (req, res) => {
        ...
        getDBUserId(req.headers)
            ...
            .then(result => {
                console.log(result);
                return queryOne(`SELECT * FROM meetups WHERE id = ?`, [
                    result.insertId
                ]);
            })
            .then(meetup => {
...
app.post(
    "/api/meetup/rejected/:id", [authCheck, guard.check("meetup:list")],
    (req, res) => {
...
app.post(
    "/api/meetup/dropped/:id", [authCheck, guard.check("meetup:list")],
    (req, res) => {
...
app.get("/api/reports", [authCheck, guard.check("report:read")], (req, res) => {
    const promises = [];
    ...
    promises.push(query(confSQL, [now()]));
    promises.push(query(meetupSQL, [now()]));
...
app.get(
    "/api/reports/todo", [authCheck, guard.check("report:due")],
    (req, res) => {
        let reportsTodo = [];
        let userId;
        getDBUserId(req.headers)
      ...
      WHERE r.sourceId = s.id
        ...
        .then(reports => {
            ...
            reports.map(r => {
                ...
                switch (r.region) {
                    case "Americas":
                        stats.monthly[month].americas += r.developersReached;
                        break;
                    case "EMEA":
                        stats.monthly[month].emea += r.developersReached;
                        break;
                    case "APAC":
                        stats.monthly[month].apac += r.developersReached;
                        break;
                    case "Global":
                        stats.monthly[month].global += r.developersReached;
                        break;
                }
                switch (r.source) {
                    case "Evangelism":
                        stats.monthly[month].evangelism += r.developersReached;
                        break;
                    case "Ambassador - External":
                        stats.monthly[month].ambext += r.developersReached;
                        break;
                    case "Ambassador - Internal":
                        stats.monthly[month].ambint += r.developersReached;
                        break;
                }
                switch (r.type) {
                    case "Conference":
                        stats.monthly[month].conference += r.developersReached;
                        break;
                    case "Meetup":
                        stats.monthly[month].conference += r.developersReached;
                        break;
